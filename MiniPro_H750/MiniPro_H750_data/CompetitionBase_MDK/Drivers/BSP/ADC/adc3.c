...
void adc3_init(void)
{
    ...
    g_adc3_handle.Init.NbrOfConversion = 1;                                          /* 赋值范围是1~16，本实验用到1个规则通道序列 */
    g_adc3_handle.Init.DiscontinuousConvMode = DISABLE;                              /* 禁止规则通道组连续采样模式 */
    g_adc3_handle.Init.NbrOfDiscConversion = 0;                                      /* 配置连续采样模式的规则通道个数，禁止规则通道组连续采样模式后，此参数忽略 */
