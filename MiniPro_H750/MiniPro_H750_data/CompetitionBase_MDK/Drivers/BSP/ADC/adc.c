...
void adc_nch_dma_init(uint32_t par, uint32_t mar)
{
    ...
    g_adc_nch_dma_handle.Init.NbrOfConversion = 6;                                          /* 赋值范围是1~16，本实验用到1个规则通道序列 */
    g_adc_nch_dma_handle.Init.DiscontinuousConvMode = DISABLE;                              /* 禁止规则通道组连续采样模式 */
    g_adc_nch_dma_handle.Init.NbrOfDiscConversion = 0;                                      /* 配置连续采样模式的规则通道个数，禁止规则通道组连续采样模式后，此参数忽略 */
