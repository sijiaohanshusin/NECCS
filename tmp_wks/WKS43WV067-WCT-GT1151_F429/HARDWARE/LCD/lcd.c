...
void display_HDBK(u16 H, u16 V, u32 background, u32 color)
{
	 u16 x,y ;
    for( x = 0 ; x < H ; x++ )	
    	 {	
						LTDC_Draw_Point(x,0,color);				 
			 }		
		
    for( y = 1 ; y < (V-1) ; y++ )
    
       {
						LTDC_Draw_Point(0,y,color);				 
    for( x = 1 ; x < (H-1) ; x++ )
    	
    	 {					
						LTDC_Draw_Point(x,y,background);				 	
    	 }		
						LTDC_Draw_Point((H-1),y,color);	
    	 }
    	
     for( x = 0 ; x < H ; x++ )
    	
    	{				
						LTDC_Draw_Point(x,(V-1),color);
    	}		
...
{   u16 x,y;
	...
	for( y = 0 ; y < 60 ; y++ )
	  for( x = 0 ; x < 800 ; x++ )
	     {
				 LTDC_Draw_Point(x,y,0xf800);
			 }
	
	for( y = 60 ; y < 120 ; y++ )
	  for( x = 0 ; x < 800 ; x++ )
	     {
				 LTDC_Draw_Point(x,y,0x07E0);
			 }
	...
	for( y = 180 ; y < 240 ; y++ )
	  for( x = 0 ; x < 800 ; x++ )
	     {
				 LTDC_Draw_Point(x,y,0xFFE0);
			 }			 
			 
	for( y = 240 ; y < 300 ; y++ )
	  for( x = 0 ; x < 800 ; x++ )
	     {
				 LTDC_Draw_Point(x,y,0x07FF);
			 }			 
			 
	for( y = 300 ; y < 360 ; y++ )
	  for( x = 0 ; x < 800 ; x++ )
	     {
				 LTDC_Draw_Point(x,y,0xF81F);
			 }	
	for( y = 360 ; y < 420 ; y++ )
	  for( x = 0 ; x < 800 ; x++ )
	     {
...
//LCD分辨率为H*V
//棋盘格的大小为M*N
void qipan_display1(u16 H, u16 V, u8 M, u8 N)
{
  u16 x=0,y=0;
  u8 i,j;
  for(j=0;j<(V/(M+N));j++)   
...
void qipan_display2(void)// H=480  V=800  M=80  N=80
{
  u32 x,y;
  u8 i,j;
  for(j=0;j<3;j++)   
  {
		for(y=160*j;y<160*j+80;y++)
    {
			for(i=0;i<6;i++)
      {
				for(x=160*i;x<160*i+80;x++)
        {   
						LTDC_Draw_Point(y,x,0x0000);  
		    }
				for(x=160*i+80;x<160*i+160;x++)
        {      
						LTDC_Draw_Point(y,x,0xFFFF);	  
		    }
       }
     }
		for(y=160*j+80;y<160*j+160;y++)
	{
			for(i=0;i<6;i++)      
     {
				for(x=160*i;x<160*i+80;x++)
    ...
    {
      {
        for(i	=	0;i	<	15;	i++)
    	     for( x = 0; x < 800; x++ )
    	      {
					LTDC_Draw_Point(x,(y*15+i),color);
				}
      }
   ...
   {
      {
        for(i	=	0;i	<	25;	i++)
    	     for( y = 0; y < 480; y++ )
    	       {
					LTDC_Draw_Point((x*25+i),y,color);
				}
      }
...
extern unsigned char girl_800480[];
