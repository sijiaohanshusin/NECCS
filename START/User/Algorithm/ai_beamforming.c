 ...
 *
 * 3. 粗搜 + 精搜策略
 *    - 粗搜：9×9 网格 (±60°, 步长 15°)，快速定位
 *    - 精搜：Top-3 峰值周围 4×4 网格 (±10°, 步长 5°)，精确定位
 ...
 *
 * 邻居定义：
 * - 在 9×9 网格中，曼哈顿距离 <= SRP_TOPK_NMS_RADIUS
 ...
 *
 * 邻域定义：
 * - 角度距离 <= SRP_CONTRAST_NEIGHBOR_EXCLUDE_DEG (默认 5°)
 * - 排除旁瓣干扰，更准确评估峰值质量
 *
 * @param   max_idx  最大值索引
 * @return  排除邻域后的次大值
 ...
 *    - 复杂度：O(N log N) = O(256 × 8) = 2048 次运算
 *    - 优化：SIMD 指令并行处理 4 个点
 *
 ...
 * 输出数据：GCC_PHAT_Buffer (40 对 × 40 bins × 2, AXI SRAM)
 *
 * @note    耗时：约 1.5ms @ 480MHz (40 对 × 40 bins)
 */
 ...
 *
 * 相位旋转公式：
 * - 频率 f_k 的相位旋转：exp(j * 2π * f_k * tau)
 * - 递推计算：exp(j * 2π * f_k * tau) = exp(j * 2π * f_0 * tau) * [exp(j * 2π * Δf * tau)]^k
 * - 优化：每对麦克风仅 4 次三角函数调用 (sin_d, cos_d, sin_phi, cos_phi)
 *
 * 算法流程：
 * 1. 计算相位增量：d_phi = 360° * Δf * tau
 ...
 * @return  该方向的 SRP 功率
 *
 * @note    耗时：约 0.03ms @ 480MHz (40 对 × 40 bins)
 */
 ...
 * - 快速扫描 81 个方向，定位大致区域
 * - 耗时：约 2.5ms (81 点 × 0.03ms/点)
 *
 * 精搜策略：
 * - 实时计算 TDOA (因为角度不固定)
 * - 在 Top-3 峰值周围细化，提升精度
 * - 耗时：约 1.5ms (48 点 × 0.03ms/点)
 *
 * 质量评估：
